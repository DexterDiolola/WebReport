DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `VIEWS`(IN `cond` VARCHAR(50), IN `routerMac` VARCHAR(50), IN `userMac` VARCHAR(50), IN `loginType` VARCHAR(50), IN `loginValue` VARCHAR(50))
    NO SQL
BEGIN
